...
/**
 * look at spinlock_ttas.hpp for detailed explanantion
 */
class spinlock
{
private:
		...
		for (;;)
		{
			...
			if (spinlock_status::locked == state_.exchange(spinlock_status::locked, std::memory_order_acquire))
			{
				std::uniform_int_distribution< std::size_t > distribution{
					...
					(std::min)(collisions, static_cast<std::size_t>(WISE_FIBERS_CONTENTION_WINDOW_THRESHOLD))
				};
