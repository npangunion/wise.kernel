...
{
	wise::kernel::mem_tracker::inst().on_dealloc(reinterpret_cast<uintptr_t>(p));
	std::free(p);
}
...
namespace wise {
	namespace kernel {
		...
		void mem_tracker::on_alloc_locked( uintptr_t mem, std::size_t size, const mstring& cls )
		{
			auto crc = crc32(cls.cbegin(), cls.cend());
